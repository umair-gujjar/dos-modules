...
class project_work(osv.osv):
    ...
    def _create_analytic_entries(self, cr, uid, vals, context):
        ...
        timeline_id = False
        acc_id = False
        prj_task_work_id = self.pool.get('project.task').browse(cr, uid, uid, context=context)
...
##################################################################################################################
        ...
        vals_line['total_no_of_entries'] = vals['entries_col']
        vals_line['check_analytic'] = vals['check_analytic']
        vals_line['product_id'] = result['product_id']
        #vals_line['task_wrk_id'] = '%s' % (tools.ustr(task_obj))
        print "********************************************"
        print "********************************************"
        print "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"
        print task_obj.work_ids
        print "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"
        print "********************************************"
        print "********************************************"
        #vals_line['task_wrko_ids'] = task_obj.work_ids.id
###################################################################################################################
        if vals.get('date'):
            if len(vals['date']) > 10:
...
########################################################################################################################
        vals_line['unit_amount'] = vals['hours']
        vals_line['total_no_of_hours'] = vals['hours']
...
##########################################################################################################################
            #task_wrk_id = task.task_id
            if 'name' in vals:
                vals_line['name'] = '%s: %s' % (tools.ustr(task.task_id.name), tools.ustr(vals['name'] or '/'))
                print "********************************************"
                print task.task_id.work_ids.id
                print "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"
                #vals_line['task_wrko_id'] = task.task_id.work_ids.id
